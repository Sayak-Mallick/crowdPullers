...
const celebImages: { id: number; src: string; alt: string; title: string; event: string; year: string }[] = [
  { id: 1, src: '/celeb/arijit.png', alt: 'Arijit Singh Live Concert', title: 'Arijit Singh Live Concert', event: 'Eco Park Cultural Festival', year: '2020' },
  { id: 2, src: '/celeb/shreya.png', alt: 'Shreya Ghoshal Performance', title: 'Shreya Ghoshal Performance', event: 'Corporate Gala Night', year: '2019' },
  { id: 3, src: '/celeb/shreya-2.png', alt: 'Shreya Ghoshal Performance 2', title: 'Shreya Ghoshal — Encore', event: 'Cultural Evening', year: '2018' },
...
export default function CelebscapeSectionClean() {
  ...
  const navigateImage = (direction: 'prev' | 'next') => {
    ...
    const newIndex = direction === 'prev' ? (currentIndex === 0 ? celebImages.length - 1 : currentIndex - 1) : (currentIndex === celebImages.length - 1 ? 0 : currentIndex + 1)
    setSelectedImage(celebImages[newIndex].id)
  }

  const handleDownloadGallery = () => {
    const galleryData = { title: 'CROWDpullers Celebrity Events Gallery', events: celebImages, downloadDate: new Date().toISOString(), totalEvents: celebImages.length }
      ...
      <div className="mx-auto max-w-7xl px-6">
        <div className="text-center mb-16">
          ...
          <p className="text-xl text-muted-foreground max-w-4xl mx-auto leading-relaxed">
            Discover our comprehensive portfolio of successfully executed events spanning over a decade - from government inaugurations and corporate galas to cultural celebrations and international exhibitions.
          </p>
        </div>
            ...
            <div key={image.id} className={cn('group relative overflow-hidden rounded-xl bg-muted cursor-pointer transition-all duration-300 hover:shadow-xl hover:scale-[1.02]', index === 0 && 'md:col-span-2 lg:col-span-1')} onClick={() => openModal(image.id)}>
              <AspectRatio.Root ratio={4 / 3}>
                <div className="relative h-full w-full flex items-center justify-center">
                  <img src={image.src} alt={image.alt} className="h-full w-full object-cover" onError={(e) => { (e.target as HTMLImageElement).src = '/crowdPullers.png' }} />
                  <div className="absolute inset-0 bg-black/20 group-hover:bg-black/40 transition-colors duration-300" />
                  <div className="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                    <Button variant="secondary" size="sm" className="gap-2"><Camera className="h-4 w-4" />View Gallery</Button>
                  </div>
                </div>
              </AspectRatio.Root>
